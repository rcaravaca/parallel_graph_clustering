...
void prefixSumPi0sIndexesPinned(int numEvents, int maxSeeds, int8_t* isMergedPi0, int* Pi0Indexes) {
    for (int event = 0; event < numEvents; ++event) {
        int counter = 0;
        for (int seed = 0; seed < maxSeeds; ++seed) {
            if (isMergedPi0[event * maxSeeds + seed] > -1) {
...
void GraphInsertionNEventsWithPi0V2(
    ...
    cudaMallocHost((void**)&rows, totalNumberOfDigits * sizeof(int));
    cudaMallocHost((void**)&cols, totalNumberOfDigits * sizeof(int));
    cudaMallocHost((void**)&energies, totalNumberOfDigits * sizeof(int));
    ...
    int *Seeds;
    cudaMallocHost((void**)&Seeds, SeedSize * sizeof(int));

    int *adjListSizes;
    cudaMallocHost((void**)&adjListSizes, numEvents * maxSeeds * sizeof(int));

    int *flatAdjList;
    cudaMallocHost((void**)&flatAdjList, adjListSize * sizeof(int));

    int *neighborsTotClE;
    cudaMallocHost((void**)&neighborsTotClE, numEvents * 64 * 58 * sizeof(int));
        
    float *flatWeights;
    cudaMallocHost((void**)&flatWeights, weightSize * sizeof(float));

    int* numSeeds;
    cudaMallocHost((void**)&numSeeds, numEvents * sizeof(int));
    ...
    int8_t *isMergedPi0;
    cudaMallocHost((void**)&isMergedPi0, numEvents * maxSeeds * sizeof(int8_t));

    int *mergedPi0AdjListSizes;
    cudaMallocHost((void**)&mergedPi0AdjListSizes, numEvents * maxSeeds * sizeof(int));

    int *expandedMergedPi0Neighbors;
    cudaMallocHost((void**)&expandedMergedPi0Neighbors, expandedMergedPi0AdjListSize * sizeof(int));
    
    int *expandedMergedPi0NumNeighbors;
    cudaMallocHost((void**)&expandedMergedPi0NumNeighbors, numEvents * maxSeeds * sizeof(int));
    
    float *expandedMergedPi0Weights;
    cudaMallocHost((void**)&expandedMergedPi0Weights, expandedMergedPi0WeightsSize * sizeof(float));
    ...
    cudaMalloc(&d_Seeds, SeedSize * sizeof(int));
    cudaMalloc(&d_numSeeds, numEvents * sizeof(int));
    cudaMalloc(&d_rows, totalNumberOfDigits * sizeof(int));
    cudaMalloc(&d_cols, totalNumberOfDigits * sizeof(int));
    cudaMalloc(&d_energies, totalNumberOfDigits * sizeof(int));
    ...
    cudaFreeHost(rows);
    cudaFreeHost(cols);
    cudaFreeHost(energies);
    cudaFreeHost(Seeds);
    cudaFreeHost(adjListSizes);
    cudaFreeHost(flatAdjList);
    cudaFreeHost(neighborsTotClE);
    cudaFreeHost(flatWeights);
    cudaFreeHost(numSeeds);
    cudaFreeHost(isMergedPi0);
    cudaFreeHost(mergedPi0AdjListSizes);
    cudaFreeHost(expandedMergedPi0Neighbors);
    cudaFreeHost(expandedMergedPi0NumNeighbors);
